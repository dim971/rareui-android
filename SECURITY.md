...
credentials and no dependencies beyond Compose itself. Every component here
draws and animates from values it is given; where the original fetches
something for itself, the port takes the data as a parameter instead, so an
application decides what is loaded and when.
